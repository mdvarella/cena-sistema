// ERP CENA — docusign-download-completed (Recovery-3)
...
serve(async (req) => {
  ...
  try {
    ...
    await registrarAuditLog(admin, fin.ok ? "FINALIZOU_DOCUSIGN" : "ERRO_DOCUSIGN", "Download/finalização envelope", {
      usuarioId: perm.usuarioId,
      usuarioNome: perm.usuarioNome,
      perfil: perm.perfil,
    }, { envelope_id: envelopeRowId, ok: fin.ok });
