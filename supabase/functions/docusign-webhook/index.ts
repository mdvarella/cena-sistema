...
import { registrarAuditLog } from "../_shared/cena-rh-auth.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!ver.ok) {
      if (ver.error === "configuration_error") {
        return json({ ok: false, error: "configuration_error" }, 503);
      }
    ...
    if (mapped) {
      ...
      if (mapped.doc) {
        const { data: docAtual } = await admin
          ...
          .maybeSingle();
        if (docAtual && mapped.doc !== "ASSINADO" && podeAvancarDoc(String(docAtual.status || ""), mapped.doc)) {
    ...
    await registrarAuditLog(admin, "RECEBEU_WEBHOOK_DOCUSIGN", "Webhook DocuSign", {
      usuarioNome: "DOCUSIGN_CONNECT",
      perfil: "sistema",
    }, { event: eventType, envelope_id: env.id });

    if (mapped?.finalize) {
      const fin = await finalizarEnvelopeDocusign(admin, env);
      await registrarAuditLog(admin, fin.ok ? "FINALIZOU_DOCUSIGN" : "ERRO_DOCUSIGN", "Webhook completed → finalizar", {
        usuarioNome: "DOCUSIGN_CONNECT",
        perfil: "sistema",
      }, { envelope_id: env.id, ok: fin.ok });
      if (!fin.ok) {
        return json({
          ok: false,
