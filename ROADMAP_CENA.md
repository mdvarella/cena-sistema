...
**Versão atual do sistema:** `8.1.101` (21/09/2026, build `20260921-1830`)  
...
# 9. Frotas
  ...
  FRO-PORT-OFF: tablet registra saída/retorno/autorizados/visitantes sem
  internet; IndexedDB + fila + UUID de idempotência; sync automático com
  horário real do evento preservado; virada do dia não esconde equipe em
  campo. SQL `sql_portaria_offline_1.sql` (manual; **não executar**).

  FRO-COMB-CONC-1: inconsistência é estado de `frotas_combustivel`
...
# 10. Programação
  ...
  Código      Item                                      Status
  ----------- ----------------------------------------- -------------------
  TMA-OPS-1   Status operacional automático das equipes 🟠 HOMOLOGAÇÃO
              (Portaria + Diário + OS na Programação TMA)
...
> trabalhar em Programação de Projetos. TMA-OPS-1 não altera Projetos /
> PLPT / RDSE / RDSC / COMGAS / SOT.
...
1.  🟠 **TMA-OPS-1 --- Status operacional automático das equipes TMA** (homologar; Portaria+Diário+OS na Programação TMA; sem SQL; sem deploy; alerta de divergência ⚠ fica para fase 2)
2.  🟠 **FRO-COMB-CONC-1 --- Combustível conciliação de placas** (homologar Fase 1; SQL manual `sql_frotas_combustivel_conciliacao_fase1.sql` **não executar**; sem criar veículo automático)
3.  🟠 **AUTH-PERSIST-1A --- Circuit breaker pós-falha Auth + fechar Realtime** (homologar; sem SQL; sem deploy)
4.  🟠 **AUTH-PERSIST-1 --- Persistência/renovação Auth + fim do loop 401 JWT expired** (homologar; sem SQL; sem deploy)
5.  🟠 **FRO-PORT-OFF --- Portaria offline-first** (homologar; SQL manual `sql_portaria_offline_1.sql` **não executar**; sem deploy)
6.  🟠 **DOC-SESMT / P1-A.4 --- CENA atual do vínculo documental** (homologar RE 240: modal lê treinamento_id do documento_final_id)
7.  🟠 **DOC-SESMT / P1-A.3 --- Escrita protegida sem fallback anon** (homologar vínculo `rh_sesmt_documento_vinculos` com JWT authenticated; AUTH-1 separado)
8.  🟠 **ALM-CORR-1 --- Corrigir item processado com aprovação do gestor** (homologar; SQL manual `sql_alm_correcao_item_solicitacoes.sql`; **não executar** RE 107 / RE 1978 automaticamente)
9.  🟠 **DOC-SESMT / P1-A.2 --- IA-5B documento_final_id (ASO certo)** (homologar RE 240)
10. 🟠 **DOC-SESMT / P1-A.1 --- Subtipo ASO + Integrar SESMT** (homologar; SQL pontual `sql_p1a1_corrigir_aso_periodico_re240.sql` + vínculo `sql_rh_sesmt_documento_vinculos.sql`)
11. 🟠 **RH-2D --- Assinatura + Dossiê / DocuSign + Kits** (homologar)
12. 🟠 **RH-SEL-1D2 --- Cargo ↔ Teste + snapshot na vaga + filtro gestor** (homologar; SQL manual)
13. 🟠 **RH-SEL-1H --- Trilha testes Eletricista I/II/III** (homologar; SQL manual)
14. 🟡 **RH-SEL-1H2 --- Trilhas por cargo/família** (SQL para revisão; **não aplicar** sem aprovação)
15. 🟠 **RH-2C --- Documentos / Templates** (homologar se pendente)
16. 🟠 **RH-2A --- Admissão Inteligente** (homologar se pendente)
17. RH-2E --- Promoção/Alteração Contratual *(não iniciar)*

> **Parada:** TMA-OPS-1 + FRO-COMB-CONC-1 + AUTH-PERSIST-1 + FRO-PORT-OFF + DOC-SESMT/P1-A + RH-2D em HOMOLOGAÇÃO. Não iniciar RH-2E sem autorização. SQL de vínculo, `sql_portaria_offline_1.sql` e `sql_frotas_combustivel_conciliacao_fase1.sql` **não** aplicar sem aprovação.
...
| 21/09/2026 | 8.1.096 | Modais: `#modal-area` recolocado no HTML — setModal não quebra mais (versão / nova multa). Sem SQL. HOMOLOGAÇÃO. |
...
| 19/09/2026 | 8.1.082 | FRO-COMB-CONC-1: conciliação de abastecimento na própria `frotas_combustivel`; importação classifica frota→Portaria→desconhecida; tela Inconsistências; sync pós-cadastro; SQL `sql_frotas_combustivel_conciliacao_fase1.sql` (não aplicar). HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.081 | IEC (DB): índice funcional + teto 25s na RPC `fn_av_calcular_avaliacao` (timeout 57014). SQL `sql_avaliacao_colaborador_perf_indexes.sql` (não aplicar). HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.080 | IEC Frota: carrega multas/avarias uma vez e filtra em memória (antes 1 busca por colaborador). HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.079 | IEC Frota: score passa a ler multas em `frotas_documentos` (tipo=Multa), não só a tabela legada vazia. HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.078 | Frotas Multas: condutor automático no dia da infração; busca nome/RE se não achar; corrige 400 PGRST204 da coluna `valor` em `frotas_documentos`. SQL opcional `sql_frotas_documentos_valor.sql` (não aplicar). HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.077 | Frotas Avarias: botão Ver abre o modal do sinistro com editar e excluir. HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.076 | IEC: avaria de Frotas (Avarias/Sinistros) entra no score Frota e na lista de detalhes do colaborador; escala de trabalho do cadastro permanece só turno. HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.075 | IEC: atraso/pontualidade usa a escala do cadastro (`escala_id` / Escala de trabalho), não o horário Nexus. HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.074 | IEC: pontualidade do gráfico usa 1ª entrada efetiva (não almoço) + tolerância 5 min; RE 1078 não cai mais por batida bruta. HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.073 | Portaria: dia 19 não mistura o dia 18; só a virada da meia-noite (saiu ontem ≥20h e cruzou 00h). HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.071 | Portaria Retorno: sem filtro auto de filial; nome da equipe do quadro; uma linha por equipe (EBN143/EBN187); virada até 3 noites. EON124/EBN196. HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.070 | Portaria: Retorno usa a mesma lista em campo da aba Saída (programação + virada da noite, casa por equipe/placa). EON124 volta em "Saíram e não retornaram". HOMOLOGAÇÃO. |
| 19/09/2026 | 8.1.069 | TMA: contrato de projeto no contexto não zera o quadro. Portaria Retorno carrega EM CAMPO de ontem→hoje. HOMOLOGAÇÃO. |
| 18/09/2026 | 8.1.068 | Frotas combustível: importação usava sbFetch (~1000 veículos) e gerava “placa não encontrada” falso; passa a sbFetchAll + norma A–Z0–9 + flag de cadastro. 🟠 HOMOLOGAÇÃO. |
| 18/09/2026 | 8.1.067 | AUTH-PERSIST-1A: circuit breaker `_cenaAuthInvalid` impede refresh após falha terminal; fecha WebSocket/heartbeat chat; reinicia realtime 1x no relogin. Sem SQL/deploy. 🟠 HOMOLOGAÇÃO. |
| 18/09/2026 | 8.1.066 | AUTH-PERSIST-1: renovação centralizada JWT, retry único em 401 expired, para loops chat/online, gate cena_user×Auth, limpeza prog_veic_*. Sem SQL/deploy. 🟠 HOMOLOGAÇÃO. |
| 18/09/2026 | 8.1.065 | Portaria: em campo/retorno só no turno (hoje + ontem ainda fora). Tira o excesso de equipes antigas sem retorno. HOMOLOGAÇÃO. |
| 18/09/2026 | 8.1.064 | FRO-PORT-OFF: Portaria offline-first (IndexedDB + fila + sync; horário real do evento; virada do dia não esconde equipe em campo). SQL manual `sql_portaria_offline_1.sql` não executado. Sem deploy. HOMOLOGAÇÃO. |
